...
class AuthManager:
    ...
    def login(self):
        ...
        def run_server():
            server = HTTPServer(('localhost', constants.OAUTH_CALLBACK_PORT), 
                              lambda *args, **kwargs: OAuthCallbackHandler(*args, auth_manager=self, **kwargs))
    ...
    def get_token(self) -> Optional[str]:
        ...
        if current_time >= config.get("expires_at", 0):
            ...
            console.print("[yellow]Session expired. Please run 'medicode login' again.[/yellow]")
            return None
